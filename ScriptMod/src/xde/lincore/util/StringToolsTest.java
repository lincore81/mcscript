...
public class StringToolsTest {
	...
	public void testGetBoolean() {
		...
		assertTrue(StringTools.getBoolean("ääääh%$%/§($)"));
	...
	public void testGetNumber() {
		...
		int i = 1;
		Map<String, Boolean> tests = new HashMap<String, Boolean>(numberTests);
		tests.putAll(integerTests);
		for (String testString: tests.keySet()) {
			System.out.format("\tcase #%d: %s", i, testString);
			Double num = StringTools.getNumber(testString);
	...
	public void testGetList() {
		System.out.println("\nTesting getList:");
		int i = 1;
		for (String testString: listTests.keySet()) {
			System.out.format("\tcase #%d: %s (valid? %b)%n", i, testString, 
					StringTools.isValidList(testString, ";"));
			i++;
			List<String> list = StringTools.getList(testString, ";");
			if (list == null) {
				System.out.println("\t\tnull");				
			}
	...
	public void testGetMap() {
		...
		int i = 1;
		for (String testString: mapTests.keySet()) {
			System.out.format("\tcase #%d: %s (valid? %b)%n", i, testString, 
	...
	public void testIsValidList() {
		...
		for (Map.Entry<String, Boolean> e : listTests.entrySet()) {			
			boolean result = StringTools.isValidList(e.getKey(), ";");
	...
	public void testIsValidMap() {
		...
		for (Map.Entry<String, Boolean> e : mapTests.entrySet()) {			
			boolean result = StringTools.isValidMap(e.getKey(), ";", "=");
			System.out.format("\tcase #%d: %b (%b expected) for \"%s\"%n", i, result, e.getValue(), e.getKey());
			assertTrue(result == e.getValue());
			i++;
		}
